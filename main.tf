...
resource "aws_cognito_identity_pool" "this" {
  identity_pool_name               = "cloudchat-identity-pool"
...
resource "aws_iam_role" "identity_pool_auth_role" {
  name = "cloudchat-identity-pool-authenticated"
...
resource "aws_iam_role_policy" "identity_pool_auth_policy" {
  name = "cloudchat-identity-pool-authenticated-policy"
...
#############################################################
resource "aws_appsync_datasource" "messages_source" {
...
resource "aws_appsync_datasource" "rooms_source" {
  api_id           = aws_appsync_graphql_api.chat_api.id
  name             = "RoomsTableDataSource"
  type             = "AMAZON_DYNAMODB"
  service_role_arn = aws_iam_role.appsync_dynamodb_role.arn
...
resource "aws_appsync_resolver" "get_messages" {
  ...
  type        = "Query"
  field       = "getMessages"
  data_source = aws_appsync_datasource.messages_source.name
...
resource "aws_appsync_resolver" "list_rooms" {
  api_id      = aws_appsync_graphql_api.chat_api.id
  type        = "Query"
  field       = "listRooms"
  data_source = aws_appsync_datasource.rooms_source.name
